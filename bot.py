...
# --- 2. CONFIGURATION ---
# Token ko dhyan se check karein
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        ...
        if referrer: users_col.update_one({"user_id": referrer}, {"$inc": {"balance": 3.0}})
    
    if await is_subscribed(context.bot, user_id): return await show_menu(update, context)
...
async def verify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await is_subscribed(context.bot, update.callback_query.from_user.id): await show_menu(update, context)
    else: await update.callback_query.answer("❌ Pehle join karo!", show_alert=True)

async def show_menu(update, context):
    menu = [[InlineKeyboardButton("💰 Wallet", callback_data="wallet"), InlineKeyboardButton("🤝 Refer", callback_data="refer")], [InlineKeyboardButton("🎁 Daily Bonus", callback_data="bonus")]]
    text = "🎯 **Aalsi Mode Active!**"
    if update.callback_query: await update.callback_query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(menu))
    else: await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(menu))

async def bonus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.callback_query.from_user.id
    user = users_col.find_one({"user_id": user_id})
    today = str(datetime.date.today())
    if user.get("last_bonus") == today: await update.callback_query.answer("Kal aana!", show_alert=True)
    else:
...
async def refer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.message.reply_text(f"🤝 Link: `t.me/{bot_un}?start={update.callback_query.from_user.id}`", parse_mode="Markdown")
