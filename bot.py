...
@flask_app.route('/')
def home(): return "Aalsi Bot: Online & Secured!", 200
...
# --- 2. CONFIGURATION ---
# Tera Naya Token yahan set kar diya hai 👇
...
# MongoDB Connection (Database Amar Hai)
...
async def verify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await is_subscribed(context.bot, update.callback_query.from_user.id):
        ...
        await update.callback_query.answer("❌ Pehle join karo bhai!", show_alert=True)
...
async def refer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.message.reply_text(f"🤝 Link: `{link}`", parse_mode="Markdown")
...
if __name__ == '__main__':
    ...
    # 'drop_pending_updates' purane conflict ko khatam kar dega
